...
namespace Discount.gRPC.Services
{
    public class DiscountService : DiscountProtoService.DiscountProtoServiceBase
    {
        ...
        {
            var coupon = _mapper.Map<Coupon>(request);
            var isModified = await _couponRepository.UpdateDiscount(coupon);
            if (isModified)
            {
                _logger.LogInformation("Discount is successfully modified. ProductName: {productName}", coupon.ProductName);
            }
            else
            {
                _logger.LogInformation("Discount modified failed.");
            }
            return _mapper.Map<CouponRequest>(coupon);
        }
        public override async Task<DeleteDiscountResponse> DeleteDiscount(DeleteDiscountRequest request, ServerCallContext context)
        {
            ...
            if (isDeleted)
            {
                _logger.LogInformation("Discount has been successfully deleted. ProductId: {ProductId}", request.ProductId);
            }
            else
            {
